...
class LLMAPI:
    ...
    llm: AsyncOpenAI = field(init=False)  # 自动创建的对象，不需要用户传入
...
async def run_api(llm: LLMAPI, data: List[str]) -> Tuple[List[str], List[LLMAPI]]:
    results = [call_llm(llm, text) for i, text in enumerate(data)]
...
if __name__ == "__main__":
    ...
    # 创建LLM
    llm = LLMAPI("", "", 0)
    results, llms = asyncio.run(run_api(llm, questions))
